...
for file in $files; do
    ...
    if [[ "${file}" == *pos_* ]]; then
        ...
        "${path_to_script}"/pos_case.sh "$path_to_args" "$output_file" >/dev/null
        res=$?
...
if [[ $pos_count_passed -eq $pos_count ]]; then
    if [[ valgrind_flag -eq 1 ]]; then
        echo -e "Positive tests passed ${GREEN}$pos_count_passed / $pos_count${RESET_COLOR} MEMORY ${GREEN}PASSED${RESET_COLOR}"
    else
...
if [[ "$neg_count" == 0 ]]; then
    ...
    if [[ valgrind_flag -eq 1 ]]; then
        echo -e "Negative tests passed ${GREEN}$neg_count_passed / $neg_count${RESET_COLOR} MEMORY ${GREEN}PASSED${RESET_COLOR}"
    else
